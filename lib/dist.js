...
const skipFirstLine = (input, callback) => {
  callback(null, input.substring(input.indexOf("\n") + 1));
};

const fixBlankNodesXML = (input, callback) => {
  callback(
    null,
    input
...
async.autoInject({
  ...
  ontologyShortcutsYaml: [_.partial(fs.readFile, path.join(srcDir, 'ac-ontology-shortcuts-fileToSignal.jsonld.yaml'), 'utf8')],
  ontologyShortcutsJsonLd: ['ontologyShortcutsYaml', async.asyncify(yaml.safeLoad)],
  // ontologyEventsTtl: ['ontologyEventsJsonLd', 'ontologyCtxt', fromJsonLdToTurtle],
  ontologyShortcutsLoaded: ['store', 'ontologyShortcutsJsonLd', 'ontologyCtxt', storeJsonLd],

  // ontologyFileMetadataYaml: [_.partial(fs.readFile, path.join(srcDir, 'ac-ontology-file-metadata.jsonld.yaml'), 'utf8')],
  // ontologyFileMetadataJsonLd: ['ontologyFileMetadataYaml', async.asyncify(yaml.safeLoad)],
  // ontologyFileMetadataLoaded: ['store', 'ontologyFileMetadataJsonLd', 'ontologyCtxt', storeJsonLd],

  fullOutputStore: [
    'store', 'ontologyLoaded', 'ontologyCollectionLoaded',
    'ontologyEventsLoaded', 'ontologyShortcutsLoaded',
    async.asyncify((store, s1, s2, s3, s4) => store)],
  ...
  outputAsQuads: ['baseAsNode', 'fullOutputStore', _.partial(rdflib.serialize, _, _, 'xxx', 'application/n-quads')],
  ...
  outputAsTurtle: ['outputAsTurtleWithEmptyPrefix', skipFirstLine],
  ...
  outputAsRdfXmlDirty: ['baseAsNode', 'fullOutputStore', _.partial(rdflib.serialize, _, _, 'xxx', 'application/rdf+xml')],
  outputAsRdfXml: ['outputAsRdfXmlDirty', fixBlankNodesXML],
